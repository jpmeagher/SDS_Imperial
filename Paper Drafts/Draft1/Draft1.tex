...
\section{Gaussian Process Regression on Phylogenies} 
...
It is also shown that for a phylogenetic Gaussian Process with this covariance function, \(Y\), there exists a set of \(n\) deterministic basis functions \(\phi_i: F \to \mathbf{R}\) and univariate Gaussian processes \(X_i\) for \(i = 1,\dots, n\) such that \[
g(f, \mathbf{t}) = \sum_{i = 1}^{n} \phi_i(f) X_i(\mathbf{t})\] has the same distribution as \(Y\). Thus, by defining an appropriate set of basis functions and Gaussian Processes, evolutionary inference can be performed on our function-valued traits.

\subsection{Basis Functions for Evolution}
